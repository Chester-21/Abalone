...
public class Platform extends javax.swing.JFrame {
	...
	/**
	 * Called when a piece is clicked in the inventory.
	 * This method selects the clicked piece for move preview/play .
	 * 
	 * @param o
	 */
	private void pieceClicked(Object o) {
	...
	private void preparePlayers() {
		...
		blackType.setEnabled(false);
		whiteType.setEnabled(false);

		//Initialize player types
		game.getBlackPlayer().setType(blackType.getSelectedItem().toString());
		game.getWhitePlayer().setType(whiteType.getSelectedItem().toString());
		//System.out.println("Black is "+blackType.getSelectedItem().toString()+" and white is "+whiteType.getSelectedItem().toString());
	}
		...
		game.addGameStartedEventListener(new GameStartedEventListener() {
			public void gameStarted(GameStartedEvent e) {
				...
				if (game.getNext().getType()=="Human") {
					...
					statusBar.setText("Black playing");
					boardPanel.repaint();
				}
			}
		}
			...
			public void movePlayed(MovePlayedEvent e) {
				...
				if (game.getNext().getType()=="Human") {
					...
					if (game.getNext().isBlack()) {
						...
						statusBar.setText("White playing");
					}
					boardPanel.repaint();
				}
			}
	...
	private void cellClicked(int x, int y) {
		if (fromCell1 == null) {
			if ((game.board[x][y].getColor() == Color.black && game.getCurrent().isBlack()) ||
					(game.board[x][y].getColor() == Color.white && !game.getCurrent().isBlack())) {
	...
	private String[] availablePlayers = Player.getAvailableTypes();
	private Game game;
	...
	private class BoardMouseListener extends MouseAdapter {
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getButton()==MouseEvent.BUTTON1) {
				...
				//System.out.println(game.getCurrent().isBlack()+" clicked "+box_x+","+box_y);
				if (click_pos < 0 || click_pos > 9) {
