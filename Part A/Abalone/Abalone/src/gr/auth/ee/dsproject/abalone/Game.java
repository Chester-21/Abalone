...
public class Game implements Runnable {
	...
	public void run() {
		...
		if (next.getType() != "Human") getNextPlayerMove();
	}
	...
	private boolean playMoveAndFeedback(Move newMove) {
		...
		if (from1==from2) { //One marble
			to2.setColor(from1.getColor());
			from1.setColor(null);
			return true;
		}
		//Two consequent marbles
		if (new Vector<Cell>(Arrays.asList(from1.getNeighbors(board))).contains(from2)) {
			...
			//Check if we are pushing a marble
	...
	/**
	 * This method is called at the beginning of the game, as well as after the first move
	 * is played.
